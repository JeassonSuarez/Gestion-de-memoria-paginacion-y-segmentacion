...
let programas = [
  ...
  let renderProgramas = () => {
    SO.memoria = encabezado + stack + heap;
    fig = `<div>
              <h2>PROGRAMAS</h2>
            </div>`;
...
const nuevoP = () => {
  $d.getElementById("nuevo").addEventListener("submit", (e) => {
    let $n = e.target;
    e.preventDefault();
    const programaNuevo = new Program($n.nombre.value, parseInt($n.codigo.value), parseInt($n.datos.value), parseInt($n.bss.value), $n.icon.value);
